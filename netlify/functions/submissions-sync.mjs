/**
 * Netlify Function: 제출 동기화 API (Blobs 저장).
 * 리다이렉트로 /api/submissions*, /api/auth/teacher/login 이 여기로 옵니다.
 */
...
const BLOB_SUBMISSIONS_KEY = 'submissions-list-v1'
const BLOB_SESSIONS_KEY = 'teacher-sessions-v1'
...
function openStore(event) {
  try {
    ...
    if (typeof blobs.getStore !== 'function') return null
    try {
      return blobs.getStore({ name: 'circuit-journal-submissions' })
    } catch {
      return blobs.getStore('circuit-journal-submissions')
    }
  } catch (e) {
    console.error('[submissions-sync] getStore failed', e)
    return null
  }
}
...
export const handler = async (event, context) => {
  ...
  if (!store) {
    return text(
      503,
      'Submission store unavailable. Ensure @netlify/blobs is installed and deploy with Netlify Blobs enabled.',
    )
  }
  ...
  try {
    ...
    console.error('[submissions-sync]', e)
