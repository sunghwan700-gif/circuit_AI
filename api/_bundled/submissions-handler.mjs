// server/submissions-handler.mjs
...
var LOCAL_KV_DIR = path.join(__dirname, "data", "kv");
...
function isRemoteKvConfigured() {
  return useVercelKv();
}

// server/submissions-store.mjs
var BLOB_SUBMISSIONS_KEY = "submissions-list-v1";
var BLOB_SESSIONS_KEY = "teacher-sessions-v1";
...
function openSubmissionsStore(_event) {
  return kvCompatibleStore();
}

// server/submissions-handler.mjs
var TOKEN_ALL = (process.env.SUBMISSIONS_API_TOKEN || "").trim();
var TOKEN_STUDENT = (process.env.SUBMISSIONS_STUDENT_TOKEN || "").trim();
var TOKEN_TEACHER = (process.env.SUBMISSIONS_TEACHER_TOKEN || "").trim();
var TEACHER_PASSWORD = (process.env.SUBMISSIONS_TEACHER_PASSWORD || process.env.VITE_TEACHER_PASSWORD || "").trim();
var TEACHERS_JSON = (process.env.SUBMISSIONS_TEACHERS_JSON || "").trim();
var TEACHER_SESSION_TTL_MS = Math.max(
  6e4,
  Number(process.env.SUBMISSIONS_TEACHER_SESSION_TTL_MS || 1e3 * 60 * 60 * 12)
);
...
async function sessionContext(store, authz) {
  ...
  return { role: "teacher", teacherId: teacherId || void 0, depts };
}
...
function normalizeRecord(incoming, existing = null) {
  ...
  const teacherFeedback = inc.teacherFeedback !== void 0 ? inc.teacherFeedback : prev.teacherFeedback;
  const feedbackUpdatedAt = inc.feedbackUpdatedAt !== void 0 ? inc.feedbackUpdatedAt : prev.feedbackUpdatedAt;
  const merged = {
    ...prev,
    ...inc,
    teacherFeedback,
...
async function handleStatus(store, event, id) {
  ...
  return json(200, {
    ...
    feedbackUpdatedAt: Number.isFinite(feedbackUpdatedAt) && feedbackUpdatedAt > 0 ? feedbackUpdatedAt : null
  });
}
function parseRoute(event) {
  ...
  if (mode === "list" || mode === "auth" || mode === "status" && rid || mode === "record" && rid) {
    return { mode, rid };
  }
  const headers = event.headers || {};
  const hget = (a, b) => headers[a] || headers[b] || "";
  const forwarded = hget("x-forwarded-uri", "X-Forwarded-Uri") || hget("x-invoke-path", "X-Invoke-Path") || "";
...
async function handleSubmissionsEvent(event) {
  ...
  if (onVercel && !isRemoteKvConfigured()) {
    return text(
      503,
      "\uC81C\uCD9C \uC800\uC7A5\uC18C(Vercel KV)\uAC00 \uC5F0\uACB0\uB418\uC5B4 \uC788\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4. Vercel \uD504\uB85C\uC81D\uD2B8 \u2192 Storage \u2192 KV\uB97C \uC0DD\uC131\xB7\uC5F0\uACB0\uD55C \uB4A4 \uC7AC\uBC30\uD3EC\uD558\uC138\uC694."
    );
  }
